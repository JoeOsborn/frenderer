...
impl engine::Game for Game {
    fn new(engine: &mut Engine) -> Self {
        ...
        let floor = Rect {
            pos: Vec2 { x: W / 2.0, y: 8.0 },
            sz: Vec2 { x: W, y: 16.0 },
        };
        let left_wall = Rect {
            pos: Vec2 { x: 8.0, y: H / 2.0 },
            sz: Vec2 { x: 16.0, y: H },
        };
        let right_wall = Rect {
            pos: Vec2 {
    ...
    fn update(&mut self, engine: &mut Engine) {
        ...
        for _iter in 0..COLLISION_STEPS {
            let guy_rect = Rect {
                pos: self.guy.pos - Vec2 { x: 8.0, y: 8.0 },
                sz: Vec2 { x: 16.0, y: 16.0 },
            };
            contacts.clear();
            ...
            contacts.extend(self.walls.iter().enumerate().filter_map(|(ri, w)| {
                Rect {
                    pos: w.pos - w.sz / 2.0,
                    sz: w.sz,
                }
                .displacement(guy_rect)
                .map(|d| (ri, d))
            }));
            dbg!(&contacts);
            ...
            for (wall_idx, _disp) in contacts.iter() {
                ...
                if self.guy.pos.x < wall.pos.x {
                    disp.x *= -1.0;
                }
                // Guy is below wall, push down
                if self.guy.pos.y < wall.pos.y {
                    disp.y *= -1.0;
                }
    ...
    fn render(&mut self, engine: &mut Engine) {
        ...
        let (text_len, _corner) = engine::BitFont::with_sheet_region(
            '0'..='9',
            Rect {
                pos: Vec2 {
                    x: 0.0,
